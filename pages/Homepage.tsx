...
const AnimatedTimetableGrid = () => {
    ...
    return (
        <GlassPanel className="p-4 md:p-6 w-full max-w-lg mx-auto relative overflow-hidden [transform:perspective(1000px)_rotateX(15deg)] group">
            ...
            <div className="absolute inset-0 animate-hero-glow bg-accent/20 blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                ...
                {slots.map((_, i) => {
                    ...
                    return (
                        <div key={i} className={`h-12 md:h-16 rounded-md animate-fade-in-up`} style={{ animationDelay }}>
                            {isOccupied && (
                                <div className={`w-full h-full rounded-md bg-accent/20 border border-accent/30 flex items-center justify-center`}>
                                   <div className="w-1/2 h-2 bg-accent/50 rounded-full" />
                                </div>
                            )}
                        </div>
                    )
...
const ProblemSolutionCard: React.FC<{ icon: React.ElementType, title: string, items: string[], isProblem?: boolean }> = ({ icon: Icon, title, items, isProblem }) => (
    <GlassPanel className={`p-6 h-full border-2 ${isProblem ? 'border-red-500/20 bg-red-900/10' : 'border-green-500/20 bg-green-900/10'}`}>
        <div className="flex items-center gap-3 mb-4">
            <Icon className={`w-6 h-6 ${isProblem ? 'text-red-400' : 'text-green-400'}`} />
            <h3 className={`text-xl font-bold ${isProblem ? 'text-red-300' : 'text-green-300'}`}>{title}</h3>
        </div>
...
const CorePillarCard: React.FC<{ icon: React.ElementType, title: string, children: React.ReactNode }> = ({ icon: Icon, title, children }) => (
    <GlassPanel className="p-8 text-center group transition-all duration-300 hover:border-accent/50 hover:-translate-y-2">
        <div className="inline-flex items-center justify-center p-4 bg-accent/10 border border-accent/20 rounded-2xl mb-4 transition-all duration-300 group-hover:scale-110 group-hover:bg-accent/20">
          <Icon className="h-10 w-10 text-accent" />
        </div>
        <h3 className="text-xl font-bold text-white">{title}</h3>
        <p className="text-text-muted mt-2 text-sm">{children}</p>
    </GlassPanel>
);
    ...
    <PublicPageLayout {...props} currentPage="Home">
        <section className="grid md:grid-cols-2 gap-12 items-center">
          <div className="text-center md:text-left animate-fade-in-right">
            <h2 className="text-3xl sm:text-4xl lg:text-6xl font-extrabold text-white">
              Intelligent Timetabling, <span className="text-accent">Simplified</span>.
            </h2>
        ...
        <section className="mt-24 sm:mt-32">
             <div className="text-center">
                <h3 className="text-sm font-bold uppercase text-accent tracking-widest">The Challenge</h3>
                <p className="text-2xl sm:text-3xl font-bold text-white mt-2">Manual scheduling is a broken puzzle.</p>
            </div>
         ...
         <section className="mt-24 sm:mt-32">
            <div className="text-center">
                <h3 className="text-sm font-bold uppercase text-accent tracking-widest">Our Solution</h3>
                <p className="text-2xl sm:text-3xl font-bold text-white mt-2">A Complete Scheduling Ecosystem</p>
            </div>
        ...
        <section className="mt-24 sm:mt-32">
            <GlassPanel className="p-8 md:p-12">
                <div className="grid md:grid-cols-2 gap-8 items-center">
                    <div>
                        <h3 className="text-2xl sm:text-3xl font-bold text-white">How it Works in 3 Simple Steps</h3>
                        <p className="text-text-muted mt-4 mb-8">We turn complexity into a straightforward, three-stage process that combines your data with our AI's power.</p>
                        <div className="space-y-6">
                            <div className="flex items-start gap-4"><div className="p-2 bg-accent/10 border border-accent/20 rounded-lg mt-1"><Lock className="w-6 h-6 text-accent"/></div><div><h4 className="font-bold text-white">1. Define Your World</h4><p className="text-sm text-text-muted">Input your subjects, faculty, rooms, and constraints. This creates the digital blueprint of your institution.</p></div></div>
                            <div className="flex items-start gap-4"><div className="p-2 bg-accent/10 border border-accent/20 rounded-lg mt-1"><Sliders className="w-6 h-6 text-accent"/></div><div><h4 className="font-bold text-white">2. Generate with AI</h4><p className="text-sm text-text-muted">Select your batches, click 'Generate', and let the AI engine produce multiple, optimized timetable candidates.</p></div></div>
                            <div className="flex items-start gap-4"><div className="p-2 bg-accent/10 border border-accent/20 rounded-lg mt-1"><BarChart4 className="w-6 h-6 text-accent"/></div><div><h4 className="font-bold text-white">3. Refine & Publish</h4><p className="text-sm text-text-muted">Compare candidates, make manual tweaks with our drag-and-drop editor, and publish the final schedule.</p></div></div>
                        </div>
                    ...
                    <div className="hidden md:block">
                        <img src="https://storage.googleapis.com/aistudio-hosting/generations/11a93b58-e4b5-4b5b-9c87-2101375d8d08/image.png" alt="A flowchart showing the three steps of the scheduling process: Define, Generate, and Refine" className="rounded-lg object-cover" />
                    </div>
                </div>
            </GlassPanel>
        </section>
