...
export const SelectionModal: React.FC<SelectionModalProps> = ({ isOpen, onClose, onConfirm, options, initialValue, mode, title }) => {
  ...
  const renderOption = (option: Option) => {
      if (mode === 'single') {
          return (
             <label key={String(option.value)} className="flex items-center space-x-3 text-sm text-white cursor-pointer p-2 rounded-md hover:bg-panel-strong">
                <input
                    type="radio"
                    name="selection-modal-option"
                    checked={isSelected(option.value)}
                    onChange={() => handleSelect(option.value)}
                    className="h-4 w-4 border-gray-300 text-[var(--accent)] focus:ring-[var(--accent)] accent-[var(--accent)] bg-panel"
                />
